...
class SwaggerParser(object):
    ...
    def __init__(self, swagger_path=None, swagger_dict=None, use_example=True):
        ...
        self.schemes = self.specification.get('shemes','')
        #end
    ...
    def build_definitions_example(self):
        ...
        for def_name, def_spec in self.specification['definitions'].items():#对definition中的每一项进行处理
    ...
    def build_one_definition_example(self, def_name):
        ...
        for prop_name, prop_spec in def_spec['properties'].items():
            ...
            if isinstance(example, bool):#返回的example是一个bool值时说明嵌套的definition已经处理了，直接跳过
                #return True 直接返回True会导致循环直接结束，不可
                example = None
    ...
    def get_example_from_prop_spec(self, prop_spec,def_name=None,def_spec=None,prop_name=None):
    ...
    def _example_from_complex_def(self, prop_spec,def_name=None,sec_name=None):
        ...
        Returns:
            An example.
        """
        if 'type' not in prop_spec['properties']:
            definition_name = self.get_definition_name_from_ref(prop_spec['properties'].values()[0]['$ref'])
            #if 'properties' in self.specification['definitions'][def_name]['properties'][sec_name].keys():
            self.definitions_example[def_name][sec_name] = {}   #初始化第二层嵌套,不要写入循环中
            for thir_name,_prop_spec in prop_spec['properties'].items():
                self.definitions_example[def_name][sec_name][thir_name] = self._example_from_definition(_prop_spec) #三层嵌套
            return True
            if self.build_one_definition_example(definition_name):
            ...
            if '$ref' in resp_spec['schema']:  # Standard definition
                ...
                if 'object' in resp_spec['schema']['type'] and 'properties' in resp_spec['schema']:
                    if '$ref' in resp_spec['schema']['properties']['data']:
                        ...
                        return [self.definitions_example[definition_name]]
                    if '$ref' in resp_spec['schema']['properties']['data']['items']:
